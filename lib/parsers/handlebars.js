...
function parseHandlebars(str, recurse) {
  ...
  // Block helpers with arguments:  {{#helper arg0 "arg1"}}template content{{/helper}}
  var blockHelperWithArgumentsStartRE = new RegExp("\\{\\{#(\\w+)\\s+((\\w+)|(" +
  ...
  while (str.length) {
    ...
    nextMatch = str.match(blockHelperWithArgumentsStartRE);
    if (nextMatch) {
